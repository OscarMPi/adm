...
class AdmsDeleteSitsUsers
{
    ...
    public function deleteSitUser(int $id): void
    {
        ...
        if(($this->viewSitUser()) and ($this->checkStatusUsed())){
            ...
            if ($deleteUser->getResult()) {
                ...
                $_SESSION['msg'] = "<p class='alert-danger'> Situação não apagada com sucesso!</p>";
                $this->result = false;
            }
        }else{
            $this->result = false;
    ...
    private function checkStatusUsed(): bool
    {
        ...
        if($viewUserAdd->getResult()){
            $_SESSION['msg'] = "<p class='alert-warning'>Situação não pode ser apagada, há usuários cadastrados com essa situação!</p>";
            return false;
        }else{
            return true;
        }
