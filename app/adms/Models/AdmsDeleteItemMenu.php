...
class AdmsDeleteItemMenu
{
    ...
    public function deleteItemMenu(int $id): void
    {
        ...
        if (($this->viewItemMenu()) and ($this->checkItemMenuUsed())) {
            ...
            if ($deleteItemMenu->getResult()) {
                ...
                $_SESSION['msg'] = "<p class='alert-danger'>Item de menu não apagado com sucesso!</p>";
                $this->result = false;
            }
        } else {
            $this->result = false;
    ...
    private function viewItemMenu(): bool
    {
        ...
        if ($this->resultBd) {
            ...
            $_SESSION['msg'] = "<p class='alert-danger'>Item de menu não encontrado!</p>";
            return false;
        }
    }
    ...
    private function checkItemMenuUsed(): bool
    {
        ...
        if ($viewItemMenuUsed->getResult()) {
            $_SESSION['msg'] = "<p class='alert-danger'>Item de menu não pode ser apagado, há permissão cadastrada com esse item de menu!</p>";
            return false;
        } else {
            return true;
        }
