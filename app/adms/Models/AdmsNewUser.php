...
class AdmsNewUser
{
    ...
    private function add(): void
    {
        if ($this->accessLevel()) {
            ...
            if ($createUser->getResult()) {
                ...
                $_SESSION['msg'] = "<p class='alert-danger'> Usuário não cadastrado com sucesso!</p>";
                $this->result = false;
            }
        } else {
            $_SESSION['msg'] = "<p class='alert-danger'> Usuário não cadastrado com sucesso 1!</p>";
            $this->result = false;
        }
    }
    ...
    private function sendEmail(): void
    {
        ...
        if ($sendEmail->getResult()) {
            $_SESSION['msg'] = "<p class='alert-success'>Sucesso: Confirmar o E-mail {$this->data['email']}!</p>";
            $this->result = true;
        } else {
            $this->fromEmail = $sendEmail->getFromEmail();
            $_SESSION['msg'] = "<p class='alert-danger'>Usuário Registrado com sucesso. E-mail Nao enviado contacte-no {$this->fromEmail}!</p>";
            $this->result = true;
        }
    }
